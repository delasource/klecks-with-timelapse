  ...
  translations.forEach(translation => {
    ...
    baseKeys.forEach(key => {
      if (!keys.includes(key)) {
        missingMessages.push(`${translation.code}: Key "${key}" not in "${translation.code}".`);
      }
    });
    keys.forEach(key => {
    ...
    if (showMissing) {
      ...
      hasMissing = true;
      console.log(`${translation.code}: ${missingMessages.length} keys missing their translation.`);
    }
  });
  if (hasMissing) {
    console.log('Show missing translations by adding "-- --missing" to the command.');
  }
  console.log(`\x1b[32m\u2714 Generated/updated ${translations.length + 2} files in "src/app/languages"\x1b[0m`);
}
