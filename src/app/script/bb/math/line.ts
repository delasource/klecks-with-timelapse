 ...
 * @param p
 */
export const projectPointOnLine = function (lineStart: TVector2D, lineEnd: TVector2D, p: TVector2D): TVector2D {
  let x, y;
...
type TBezierLineControlsCallback = (v: { p1: TVector2D; p2: TVector2D; p3: TVector2D; p4: TVector2D }) => void;
...
export class BezierLine {
   ...
   */
  private getBezierPoints(p1: TVector2D, p2: TVector2D, p3: TVector2D, p4: TVector2D, resolution: number): TVector2D[] {
    const curvePoints = [];
    let t;
  ...
  addFinal(spacing: number, callback?: TBezierLineCallback, controlsCallback?: TBezierLineControlsCallback): void {
...
export function powerSplineInput(
  ...
  for (let i = 0; i <= 1; i += stepSize) {
    resultArr.push([round(i, 4), round(startVal + Math.pow(i, exponent) * (endVal - startVal), 4)]);
  }
  return resultArr;
}
