...
import { TPointerButton, TPointerEvent, TPointerEventType, TPointerType, TWheelEvent } from './event.types';
 ...
 */
function correctPointerEvent(event: PointerEvent | TExtendedDOMPointerEvent): TCorrectedPointerEvent {
  ...
  if (IS_FIREFOX && event.pointerType != 'mouse' && event.type === 'pointermove' && event.buttons === 0) {
    // once again firefox
  ...
  constructor(p: TPointerListenerParams) {
    ...
    const finalizeWheelEvent = (e: WheelEvent | TWheelCleanerEvent): void => {
      ...
      const whlEvent: TWheelEvent = {
        ...(e instanceof WheelEvent ? { deltaY: e.deltaY / 120, pageX: e.pageX, pageY: e.pageY } : e),
    ...
    if (this.onPointerCallback) {
      ...
      this.onPointerDown = (event: PointerEvent, onSkipGlobal?: boolean) => {
        ...
        const outEvent: TPointerEvent = this.createPointerOutEvent('pointerdown', correctedEvent, {
          downPageX: correctedEvent.pageX,
          downPageY: correctedEvent.pageY,
