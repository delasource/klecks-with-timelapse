...
export function testShouldPixelate(
  ...
  return (
    ((Math.abs(width) % 2 === 0 && transform.x % 1 === 0) || (Math.abs(width) % 2 === 1 && transform.x % 1 === 0.5)) &&
    ((Math.abs(height) % 2 === 0 && transform.y % 1 === 0) || (Math.abs(height) % 2 === 1 && transform.y % 1 === 0.5))
  );
}
 ...
 */
export function canvasBounds(context: CanvasRenderingContext2D, integerBounds?: TBounds): TRect | undefined {
...
export function drawSelectionMask(selection: MultiPolygon, context: CanvasRenderingContext2D): void {
