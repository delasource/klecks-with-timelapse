...
export class KlApp {
  ...
  constructor(p: TKlAppParams) {
    ...
    this.uiLayout = (
      this.embed ? 'left' : LocalStorage.getItem('uiState') ? LocalStorage.getItem('uiState') : 'right'
    ) as TUiLayout;
    ...
    const initialWidth = Math.max(
      10,
      Math.min(maxCanvasSize, window.innerWidth < this.collapseThreshold ? this.uiWidth : this.uiWidth - this.toolWidth)
    );
    ...
    let currentLayer: TKlCanvasLayer = this.klCanvas.getLayer(this.klCanvas.getLayerCount() - 1);
    ...
    const propagateUndoRedoChanges = (type: THistoryExecutionType, composedBefore: THistoryEntryDataComposed) => {
    ...
    const klAppSelect = new KlAppSelect({
      ...
      onFill: () => {
        ...
        this.statusOverlay.out(this.klCanvas.getSelection() ? LANG('filled-selected-area') : LANG('filled'), true);
      },
      onErase: () => {
        const layerIndex = currentLayer.index;
        this.klCanvas.eraseLayer({
    ...
    const keyListener = new BB.KeyListener({
      onDown: (keyStr, event, comboStr) => {
        ...
        if (
          ...
          ((BB.sameKeys('ctrl+shift+z', comboStr) || BB.sameKeys('cmd+shift+z', comboStr)) && keyStr === 'z')
        ) {
          event.preventDefault();
          redo();
        }
        ...
        if (comboStr === 'sqbr_open') {
          currentBrushUi.decreaseSize(Math.max(0.005, 0.03 / this.easel.getTransform().scale));
        }
        if (comboStr === 'sqbr_close') {
          currentBrushUi.increaseSize(Math.max(0.005, 0.03 / this.easel.getTransform().scale));
        }
        if (comboStr === 'enter') {
          if (!applyUncommitted()) {
            this.klCanvas.layerFill(currentLayer.index, this.klColorSlider.getColor(), undefined, true);
    ...
    const fileUi = this.embed
      ...
      : new KL.FileUi({
          ...
          onUpload: () => {
            // on upload
            applyUncommitted();
            KL.imgurUpload(this.klCanvas, this.rootEl, p.app && p.app.imgurKey ? p.app.imgurKey : '', () =>
              this.updateLastSaved()
            );
          },
          applyUncommitted: () => applyUncommitted(),
    ...
    if (!this.embed || this.embed.enableImageDropperImport) {
      ...
      window.document.addEventListener('paste', (e: ClipboardEvent) => importHandler.onPaste(e), false);
    }
