...
export class KlAppImportHandler {
  ...
  private importFinishedLoading(
    ...
    const importAsImagePsd = (convertedPsdObj: TKlPsd, cropObj?: TRect) => {
      this.applyUncommitted();
      // crop
      const crop = (targetCanvas: HTMLCanvasElement, cropCanvas: HTMLCanvasElement, cropObj: TRect): void => {
      ...
      KL.showImportAsLayerDialog({
        ...
        callback: (transformObj, isPixelated?: boolean) => {
          ...
          const operation = (ctx: CanvasRenderingContext2D) => {
            BB.drawTransformedImageWithBounds(ctx, canvas, transformObj, undefined, isPixelated);
          };
          if (
  ...
  onPaste(e: ClipboardEvent): void {
    ...
    const retrieveImageFromClipboardAsBlob = (items: DataTransferItemList, callback: (file: File) => void) => {
      if (!items) {
        return;
      }
                ...
                try {
                  ...
                  if (psd.width > maxResolution || psd.height > maxResolution) {
                    ...
                    KL.popup({
                      ...
                      type: 'error',
                      message:
                        LANG('import-psd-too-large').replace(/{x}/g, '' + maxResolution) +
                        '<br /><br />' +
