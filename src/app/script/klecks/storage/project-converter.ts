...
export class ProjectConverter {
  ...
  static async readStorageProject(storageProject: TKlStorageProjectRead): Promise<TReadStorageProjectResult> {
    if (
      ...
      layers: (
        await Promise.all(
          storageProject.layers.map(layer => {
            if (layer.blob) {
              return loadImage(layer.blob).catch(() => new Image(storageProject.width, storageProject.height));
            }
