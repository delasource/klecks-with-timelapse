...
export class KlEventReplayer {
   ...
   * @private
   */
  private processUndo(finalEvents: TRecordedEvent[], undoStack: TRecordedEvent[], redoStack: TRecordedEvent[]): void {
   ...
   * @private
   */
  private processRedo(finalEvents: TRecordedEvent[], undoStack: TRecordedEvent[], redoStack: TRecordedEvent[]): void {
  ...
  private calculateTimingParams(eventCount: number, config: TReplayConfig) {
    ...
    if (DEBUG_REPLAYER) {
      console.log(
        '%c[REPLAY]',
        LOG_STYLE_REPLAYER,
        `Timings: targetFps=${targetFps}, replayTime=${replayTime}ms, totalFrames=${totalFrameCount}, eventsPerFrame=${eventsPerFrame}, timePerFrame=${timePerFrame.toFixed(2)}ms`
      );
    }
