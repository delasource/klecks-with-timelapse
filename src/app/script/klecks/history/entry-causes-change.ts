...
export function entryCausesChange(entry: THistoryEntryData, composed: THistoryEntryDataComposed): boolean {
  ...
  if (entry.layerMap !== undefined) {
    ...
    for (const layerId of entryLayerMapIds) {
      ...
      if (entryLayer.isVisible !== undefined && entryLayer.isVisible !== composedLayer.isVisible) {
        return true;
      }
      if (entryLayer.mixModeStr !== undefined && entryLayer.mixModeStr !== composedLayer.mixModeStr) {
        return true;
      }
