...
export class KlChainRecorder {
  ...
  chainIn(event: TDrawEvent): TDrawEvent | null {
    if (this.isChainDisabled) return null; // While replaying animation is active, do not process new events
  ...
  private translateToStringRepresentation(event: TDrawEvent): TSanitizedDrawEvent | null {
    ...
    if (event.type == 'down') {
      return (
        (event.shiftIsPressed ? 'D' : 'd') +
        `${rCoordinate(event.x)}|${rCoordinate(event.y)}|${rPercentage(event.pressure)}@${rPercentage(event.scale)}`
      );
    }
    if (event.type == 'move') {
      return (
        (event.shiftIsPressed ? 'M' : 'm') +
        `${rCoordinate(event.x)}|${rCoordinate(event.y)}|${rPercentage(event.pressure)}@${rPercentage(event.scale)}`
      );
    }
    if (event.type == 'up') {
      return (event.shiftIsPressed ? 'U' : 'u') + `@${rPercentage(event.scale)}`;
    }
    if (event.type == 'line') {
      return (
        'L' +
  ...
  emitReplayedEvent(drawEvents: TSanitizedDrawEvent[]) {
    if (!this.chainOut) return; // Chain disabled
