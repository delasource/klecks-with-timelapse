...
export class KlHistory {
  ...
  private updateComposed(): void {
    this.composed = composeHistoryStateData(this.entries.slice(0, this.index + 1).map(item => item.data));
  }
  ...
  push(entryData: THistoryEntryData, replaceTop?: boolean): void {
    ...
    if (replaceTop && this.index > 0) {
      ...
      const isDifferent = entryCausesChange(
        entryData,
        composeHistoryStateData(this.entries.slice(0, this.index + 1).map(item => item.data))
      );
    ...
    while (oldestIndex > 0) {
      ...
      if (this.entries.reduce((before, item) => before + item.memoryEstimateBytes, 0) > this.totalThresholdBytes) {
        oldestIndex = 1;
      }
    }
