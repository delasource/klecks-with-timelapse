...
export class PenBrush {
  ...
  private updateChangedTiles(bounds: TBounds) {
    ...
    this.changedTiles = updateChangedTiles(
      this.changedTiles,
      getChangedTiles(boundsWithinSelection, this.context.canvas.width, this.context.canvas.height)
    );
  }
  ...
  private updateAlphaCanvas() {
    ...
    for (let i = 0; i < instructionArr.length; i++) {
      ...
      ctx.drawImage(ALPHA_IM_ARR[this.settingAlphaId], 0, 0, instructionArr[i][1], instructionArr[i][1]);
  ...
  private drawDot(
    ...
    const boundsSize =
      this.settingAlphaId === ALPHA_CIRCLE || this.settingAlphaId === ALPHA_CAL ? size : size * Math.sqrt(2);
  ...
  startLine(x: number, y: number, p: number): void {
    ...
    this.selectionBounds = this.selection ? integerBounds(getMultiPolyBounds(this.selection)) : undefined;
  ...
  drawLineSegment(x1: number, y1: number, x2: number, y2: number): void {
    ...
    for (loopDist = this.lineToolLastDot; loopDist <= mouseDist; loopDist += bdist) {
      this.drawDot(x1 + eX * loopDist, y1 + eY * loopDist, this.settingSize, this.settingOpacity, localScatter, angle);
    }
    this.context.restore();
