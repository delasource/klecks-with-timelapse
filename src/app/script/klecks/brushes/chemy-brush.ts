...
export class ChemyBrush {
  ...
  private updateCompleteRedrawBounds(x: number, y: number): void {
    ...
    const boundsWithinSelection = this.selectionBounds ? boundsOverlap(bounds, this.selectionBounds) : bounds;
    if (!boundsWithinSelection) {
      return;
    }
    this.completeRedrawBounds = BB.updateBounds(this.completeRedrawBounds, boundsWithinSelection);
  }
