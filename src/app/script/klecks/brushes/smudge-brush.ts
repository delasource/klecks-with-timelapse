...
export class SmudgeBrush {
  ...
  copyFromCanvas(): void {
    ...
    touchedCells.forEach((item, i) => {
      ...
      for (let i = 0; i < h; i++) {
        for (
          let e = 0, e2 = i * w * 4, e3 = ((y * CELL_SIZE + i) * this.copyImageData.width + x * CELL_SIZE) * 4;
          e < w;
          e++, e2 += 4, e3 += 4
        ) {
  ...
  startLine(x: number, y: number, p: number): void {
    ...
    this.mask = selection ? getBinaryMask(selection, this.context.canvas.width, this.context.canvas.height) : undefined;
    p = BB.clamp(p, 0, 1);
    ...
    const totalCells =
      Math.ceil(this.context.canvas.width / CELL_SIZE) * Math.ceil(this.context.canvas.height / CELL_SIZE);
