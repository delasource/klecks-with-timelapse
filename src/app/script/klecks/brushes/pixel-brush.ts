...
export class PixelBrush {
  ...
  private updateChangedTiles(bounds: TBounds) {
    ...
    const changedTiles = getChangedTiles(boundsWithinSelection, this.context.canvas.width, this.context.canvas.height);
  ...
  private continueLine(x: number | null, y: number | null, size: number, pressure: number): void {
    ...
    const controlCallback = (controlObj: { p1: TVector2D; p2: TVector2D; p3: TVector2D; p4: TVector2D }): void => {
