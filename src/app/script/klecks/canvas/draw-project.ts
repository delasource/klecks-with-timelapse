...
export function drawProject(project: TKlProject, factor: number, selection?: MultiPolygon): HTMLCanvasElement {
  ...
  const transformedSelection = selection
    ? transformMultiPolygon(selection, scale(resultCanvas.width / project.width, resultCanvas.height / project.height))
    : undefined;
