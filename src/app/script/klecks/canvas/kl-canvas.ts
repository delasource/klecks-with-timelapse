...
import { THistoryEntryDataComposed, THistoryEntryLayerComposed, TLayerId } from '../history/history.types';
...
export class KlCanvas {
  ...
  private transformSelectionAndSample(transformation: Matrix): void {
    ...
    this.selectionSample.transformation = compose([transformation, this.selectionSample.transformation]);
  }
  ...
  resize(w: number, h: number, algorithm: 'smooth' | 'pixelated' = 'smooth'): boolean {
    ...
    if (this.selection) {
      this.selection = transformMultiPolygon(this.selection, scale(w / this.width, h / this.height));
    }
    this.width = w;
    this.height = h;
   ...
   * crop / extend
   */
  resizeCanvas(p: { left: number; top: number; right: number; bottom: number; fillColor?: TRgb }): void {
      ...
      this.klHistory.push({
        ...
        layerMap: createLayerMap(
          ...
          { attributes: ['index'] },
          {
            layerId,
            attributes: 'all',
            tiles: data ? undefined : createFillColorTiles(this.width, this.height, 'transparent'),
          }
        ),
      });
    }
...

    {
      ...
      srcComposed.tiles.forEach((tile, index) => {
        ...
        if (isLayerFill(tile)) {
          ...
          ctx.fillRect(x * HISTORY_TILE_SIZE, y * HISTORY_TILE_SIZE, HISTORY_TILE_SIZE, HISTORY_TILE_SIZE);
        } else {
  ...
  mergeLayers(layerBottomIndex: number, layerTopIndex: number, mixModeStr: TMixMode | 'as-alpha'): void | number {
    if (!this.layers[layerBottomIndex] || !this.layers[layerTopIndex] || layerBottomIndex === layerTopIndex) {
      return;
    }
    //order messed up
  ...
  flip(isHorizontal: boolean, isVertical: boolean, layerIndex?: number): void {
    ...
    this.klRecorder?.record('l-flip', [isHorizontal, isVertical, layerIndex]);
    this.klHistory.push({
      layerMap: createLayerMap(
        this.layers,
        targetLayer ? { layerId: targetLayer.id, attributes: ['tiles'] } : { attributes: ['tiles'] }
      ),
      ...(this.selection ? { selection: { value: this.selection } } : {}),
    });
  }
  ...
  layerFill(layerIndex: number, colorObj: TRgb, compositeOperation?: string, doClipSelection?: boolean): void {
    ...
    if (!this.klHistory.isPaused()) {
      ...
      this.klHistory.push({
        layerMap: createLayerMap(this.layers, {
          layerId: targetLayer.id,
          attributes: ['tiles'],
          tiles: isUniformFill ? createFillColorTiles(this.width, this.height, fill) : undefined,
          bounds,
        }),
      });
    }
  ...
  floodFill(
    ...
    const selectionMask = this.selection ? getBinaryMask(this.selection, this.width, this.height) : undefined;
    ...
    if (!this.klHistory.isPaused()) {
      this.klRecorder?.record('flood-fill', [layerIndex, x, y, rgb, opacity, tolerance, sampleStr, grow, isContiguous]);
      this.klHistory.push({
        layerMap: createLayerMap(this.layers, {
          layerId: targetLayer.id,
          attributes: ['tiles'],
  ...
  drawShape(layerIndex: number, shapeObj: TShapeToolObject): void {
    ...
    const targetLayer = this.layers[layerIndex];
    const selectionPath = this.selection ? new Path2D(getSelectionPath2d(this.selection)) : undefined;
    ...
    if (!this.klHistory.isPaused()) {
      ...
      this.klHistory.push({
        layerMap: createLayerMap(this.layers, {
          layerId: targetLayer.id,
          attributes: ['tiles'],
          tiles: isUniformFill ? createFillColorTiles(this.width, this.height, 'transparent') : undefined,
          bounds,
        }),
      });
    }
  ...
  transformViaSelection(p: {
    ...
    if (!this.klHistory.isPaused()) {
      ...
      const targetLayer =
        p.targetLayer !== undefined && p.targetLayer !== p.sourceLayer ? this.layers[p.targetLayer] : undefined;
      ...
      this.klHistory.push({
        ...
        layerMap: createLayerMap(
          this.layers,
          {
            layerId: srcLayer.id,
            attributes: ['tiles'],
            bounds: srcAndTargetEqual ? BB.updateBounds(srcBounds, targetBounds) : srcBounds,
          },
          targetLayer
            ? {
                layerId: targetLayer.id,
