...
import { CoalescedExploder, TCoalescedPointerEvent } from '../../../../bb/input/event-chain/coalesced-exploder';
...
export class EaselBrush implements TEaselTool {
  ...
  private onExplodedPointer(e: TCoalescedPointerEvent): void {
    ...
    if (!e.isCoalesced) {
      ...
      this.currentCursor.update(this.easel.getTransform(), { x: e.relX, y: e.relY }, this.radius);
  ...
  activate(cursorPos?: TVector2D): void {
    ...
    if (cursorPos) {
      ...
      this.currentCursor.update(this.easel.getTransform(), { x: cursorPos.x, y: cursorPos.y }, this.radius);
    } else {
      this.onPointerLeave();
    }
  }
