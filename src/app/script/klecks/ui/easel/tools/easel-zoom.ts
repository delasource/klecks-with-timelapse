...
export class EaselZoom implements TEaselTool {
  ...
  onPointer(e: TPointerEvent): void {
    ...
    if (e.type === 'pointermove' && ['left'].includes(e.button!) && this.downPos && this.downTransform) {
      ...
      this.easel.setTransform(createTransform(viewportPoint, canvasPoint, newScale, this.downTransform.angleDeg), true);
