...
import { TArrowKey, TEaselInterface, TEaselProject, TEaselTool, TEaselToolTrigger } from './easel.types';
...
function getToolEntries<GToolId extends string>(tools: Record<GToolId, TEaselTool>): [GToolId, TEaselTool][] {
...
export class Easel<GToolId extends string> {
  ...
  private readonly onTransformChange: (transform: TViewportTransform, scaleOrAngleChanged: boolean) => void;
  ...
  private updateToolSvgs(): void {
    ...
    Object.keys(this.toolsMap).forEach(toolId => {
      this.toolsMap[toolId as GToolId].getSvgElement().style.display = toolId === tool ? '' : 'none';
    });
  }
  ...
  private updateDoubleTapPointerTypes(): void {
    const pointerTypes =
      this.toolsMap[this.tempTool ?? this.tool].doubleTapPointerTypes ?? DEFAULT_DOUBLE_TAP_POINTER_TYPES;
  ...
  private renderLoop(): void {
    ...
    const isPositionChanged =
      newTransform.x !== this.lastRenderedTransform.x || newTransform.y !== this.lastRenderedTransform.y;
  ...
  private getFitTransform(): TViewportTransform {
    ...
    matrix = BB.Matrix.multiplyMatrices(matrix, BB.Matrix.createRotationMatrix((newAngleDeg / 180) * Math.PI));
    ...
    //fit bounds
    const padding = 0;
    const { width: fitWidth } = BB.fitInto(boundsWidth, boundsHeight, this.width - padding, this.height - padding, 1);
    ...
    return createTransform(viewportCenterP, { x: projectWidth / 2, y: projectHeight / 2 }, factor, newAngleDeg);
  }
  ...
  constructor(p: TEaselParams<GToolId>) {
    ...
    this.pointerPreprocessor = new EaselPointerPreprocessor({
      ...
      onPinch: event => {
        if (event.type === 'move') {
          ...
          newAngleDeg = minimizeAngleDeg(snapAngleDeg(newAngleDeg, 90, angleIsExtraSticky ? 12 : 4));
          if (newAngleDeg % 90 !== 0) {
    ...
    this.pointerListener = new PointerListener({
      ...
      onWheel: e => {
        ...
        const mat = createMatrixFromTransform(transform);
        const canvasPoint = applyToPoint(inverse(mat), viewportPoint);
        const newScale = BB.clamp(transform.scale * Math.pow(1 + 2 / 10, -e.deltaY), EASEL_MIN_SCALE, EASEL_MAX_SCALE);
        this.setTargetTransform(createTransform(viewportPoint, canvasPoint, newScale, transform.angleDeg), isImmediate);
      },
      onEnterLeave: isOver => {
        const tool = this.getActiveTool();
        if (!isOver) {
    ...
    this.keyListener = new KeyListener({
      onDown: (keyStr, e, comboStr, isRepeat) => {
        ...
        if (comboStr === 'plus' || comboStr === 'equals') {
          ...
          const newScale = zoomByStep(oldScale, this.keyListener.isPressed('shift') ? 1 / 8 : 1 / 4);
        ...
        TEMP_TRIGGERS_KEYS.forEach(keyTrigger => {
          if (comboStr === keyTrigger && this.toolsMap[this.tool].blockTrigger !== keyTrigger) {
      ...
      onUp: (keyStr, e, oldComboStr) => {
        ...
        TEMP_TRIGGERS_KEYS.forEach(keyTrigger => {
          if (keyStr === keyTrigger && this.toolsMap[this.tool].blockTrigger !== keyTrigger) {
  ...
  scale(factor: number, viewportX?: number, viewportY?: number): void {
    ...
    metaTransform.scale = BB.clamp(metaTransform.scale * factor, EASEL_MIN_SCALE, EASEL_MAX_SCALE);

    this.setTargetTransform(
      createTransform(metaTransform.viewportP, metaTransform.canvasP, metaTransform.scale, metaTransform.angleDeg)
    );
  }
  ...
  setAngleDeg(angleDeg: number, isRelative: undefined | boolean) {
    ...
    const newAngleDeg = minimizeAngleDeg(isRelative ? viewportTransform.angleDeg + angleDeg : angleDeg);
