...
export function newImageDialog(p: {
  ...
  function createRatioSize(ratioX: number, ratioY: number, width: number, height: number, padding: number): TSize2D {
    return BB.fitInto(
      ratioX,
      ratioY,
  ...
  const preview = BB.el({
    ...
    css: {
      ...
      height: 100 + 'px',
      backgroundColor:
        'rgba(' + backgroundRgba.r + ',' + backgroundRgba.g + ',' + backgroundRgba.b + ', ' + backgroundRgba.a + ')',
      marginLeft: 'auto',
      marginRight: 'auto',
      color: '#aaa',
      fontSize: '16px',
