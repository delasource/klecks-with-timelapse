...
export class RecoveryManagerPanel {
  ...
  private readonly recoveryListener = (metas: TRecoveryMetaData[], totalMemoryUsedBytes: number) => {
  ...
  async update(metas: TRecoveryMetaData[], totalMemoryUsedBytes: number): Promise<void> {
    ...
    metas
      ...
      .forEach(meta => {
        ...
        const infoEl = BB.el({
          content: [timestampToAge(meta.timestamp), ', ' + BB.round(meta.memoryEstimateBytes / 1000000, 1) + ' MB'],
          css: {
            textAlign: 'center',
          },
        });
