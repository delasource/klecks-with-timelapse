...
export class TextToolViewportUI {
  ...
  constructor(p: TViewportUIParams) {
    ...
    this.inputsRootEl = c(',flex,gap-5', [this.rotationSlider.getElement(), c(), this.zoomInBtn, this.zoomOutBtn]);
  }
  ...
  render(): void {
    ...
    const padding = 100;
    const fitBounds = BB.fitInto(bounds.width, bounds.height, this.width - padding, this.height - padding);
...

    {
      ...
      this.layersCtx.globalAlpha =
        this.layerArr[this.layerIndex].opacity * (this.layerArr[this.layerIndex].isVisible ? 1 : 0);
