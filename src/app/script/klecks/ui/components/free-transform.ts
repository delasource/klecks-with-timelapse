...
export class FreeTransform {
  ...
  private restoreRatio(widthChanged: boolean, heightChanged: boolean): void {
    ...
    if (heightChanged && !widthChanged) {
      ...
      if (angle90) {
        newWidth =
          (whSwapped ? this.value.y % 1 : this.value.x % 1) === 0 ? BB.roundEven(newWidth) : BB.roundUneven(newWidth);
      }
    ...
    if (!heightChanged && widthChanged) {
      ...
      if (angle90) {
        newHeight =
          (whSwapped ? this.value.x % 1 : this.value.y % 1) === 0 ? BB.roundEven(newHeight) : BB.roundUneven(newHeight);
      }
