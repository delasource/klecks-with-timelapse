...
import editCropImg from '../../../../img/ui/edit-crop.svg';
  ...
  constructor(p: TCropCopyParams) {
    ...
    pointerChain.setChainOut(event => {
      if (event.type === 'pointerdown' && event.button === 'left') {
        ...
        if (startCrop) {
          ...
          this.cropRect.x = BB.clamp(this.cropRect.x, 0, this.canvas.width - this.cropRect.width);
          this.cropRect.y = BB.clamp(this.cropRect.y, 0, this.canvas.height - this.cropRect.height);
        } else {
    ...
    this.keyListener = new BB.KeyListener({
      onDown: (keyStr, e) => {
        ...
        if (keyStr === 'left') {
          if (shiftIsPressed) {
            this.cropRect.width = BB.clamp(this.cropRect.width - stepSize, 1, this.canvas.width - this.cropRect.x);
          } else {
            this.cropRect.x = BB.clamp(this.cropRect.x - stepSize, 0, this.canvas.width - this.cropRect.width);
          }
          doUpdate = true;
        }
        if (keyStr === 'right') {
        ...
        if (keyStr === 'up') {
          if (shiftIsPressed) {
            this.cropRect.height = BB.clamp(this.cropRect.height - stepSize, 1, this.canvas.height - this.cropRect.y);
          } else {
            this.cropRect.y = BB.clamp(this.cropRect.y - stepSize, 0, this.canvas.height - this.cropRect.height);
          }
          doUpdate = true;
        }
        if (keyStr === 'down') {
