...
export class Options<IdType> {
  ...
  private update(): void {
    for (let i = 0; i < this.optionArr.length; i++) {
      this.optionArr[i].el.classList.toggle('kl-option-selected', this.optionArr[i].id === this.selectedId);
    }
  }
  ...
  constructor(p: {
    ...
    const createOption = (o: { id: IdType; label: string | HTMLElement | SVGElement; title?: string }) => {
