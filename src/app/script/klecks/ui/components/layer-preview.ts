...
export class LayerPreview {
  ...
  private updateLabel(): void {
    ...
    if (this.layer.isVisible) {
      this.opacityEl.innerHTML = LANG('opacity') + '<br>' + Math.round(this.layer.opacity * 100) + '%';
    } else {
      this.opacityEl.innerHTML = LANG('opacity') + '<br><s>' + Math.round(this.layer.opacity * 100) + '%</s>';
    }
  }
