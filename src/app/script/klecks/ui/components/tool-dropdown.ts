...
export class ToolDropdown {
  ...
  private readonly optionArr: TToolType[] = ['brush', 'paintBucket', 'gradient', 'text', 'shape', 'select'];
  ...
  private updateButton() {
    this.activeButton.title =
      this.titleArr[this.currentActiveIndex] + (this.isActive ? ' (' + LANG('tool-more-tools-click-again') + ')' : '');
  ...
  constructor(p: { onChange: (activeStr: TToolType) => void }) {
    this.imArr = [toolPaintImg, toolFillImg, toolGradientImg, toolTextImg, toolShapeImg, toolSelectImg];
    this.titleArr = [
      `${LANG('tool-brush')} [B]`,
