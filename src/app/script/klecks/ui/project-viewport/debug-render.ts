...
export type TDebugRenderFunc = (ctx: CanvasRenderingContext2D, width: number, height: number, scale: number) => void;
  ...
  constructor() {
    if (DEBUG_RENDERER_ENABLED) {
      ...
      const pattern = throwIfNull(
        BB.ctx(BB.canvas(HISTORY_TILE_SIZE, HISTORY_TILE_SIZE)).createPattern(checkerCanvas, 'repeat')
      );
