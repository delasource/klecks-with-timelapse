...
export class FxPreviewRenderer {
  ...
  render: TProjectViewportLayerFunc = (viewportTransform, viewportWidth, viewportHeight) => {
    ...
    if (!this.texture || JSON.stringify(onUpdateProps) !== JSON.stringify(this.oldOnUpdateProps) || this.postMix) {
      ...
      const propsDidChange = JSON.stringify(onUpdateProps) !== JSON.stringify(this.oldOnUpdateProps);
      ...
      if (this.selection && (!this.maskTexture || propsDidChange)) {
        if (!this.maskCanvas) {
          this.maskCanvas = BB.canvas(onUpdateProps.textureWidth, onUpdateProps.textureHeight);
        } else {
