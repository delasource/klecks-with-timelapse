...
export function blendTransform(
  ...
  // approximately equal
  if (
    (currentTransform.x === targetTransform.x || Math.abs(currentTransform.x - targetTransform.x) < 2) &&
    (currentTransform.y === targetTransform.y || Math.abs(currentTransform.y - targetTransform.y) < 2) &&
  ...
  if (BB.Vec2.dist(currentMeta.canvasP, targetMeta.canvasP) < 1) {
    ...
    return createTransform(mixedMeta.viewportP, mixedMeta.canvasP, mixedMeta.scale, mixedMeta.angleDeg);
  }
