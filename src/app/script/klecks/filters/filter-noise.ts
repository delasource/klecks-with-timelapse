...
import { TFilterApply, TFilterGetDialogParam, TFilterGetDialogResult, TMixMode, TRgb } from '../kl-types';
...
export const filterNoise = {
  getDialog(params: TFilterGetDialogParam) {
    ...
    row1El.append(channelsOptions.getElement(), BB.el({ css: { flexGrow: '1' } }), reverseToggle.getElement());
  ...
  apply(params: TFilterApply<TFilterNoiseInput>): boolean {
    ...
    klHistory.push(
      getPushableLayerChange(
        ...
        canvasToLayerTiles(context.canvas, selection ? integerBounds(getMultiPolyBounds(selection)) : undefined)
      )
    );
    return true;
  },
