...
import { isLayerFill, TFilterApply, TFilterGetDialogParam, TFilterGetDialogResult } from '../kl-types';
...
function drawTransform(
  ...
  if (selection) {
    ...
    if (
      isPixelated ||
      BB.testShouldPixelate(transform, transform.width / bounds.width, transform.height / bounds.height)
    ) {
    ...
    matrix = compose(
      ...
      scale(Math.abs(transform.width / bounds.width), Math.abs(transform.height / bounds.height)),
...
export const filterTransform = {
  getDialog(params: TFilterGetDialogParam) {
    ...
    const boundsObj = selection ? getSelectionBounds(selection, context) : BB.canvasBounds(context);
    if (!boundsObj) {
      return {
        error: LANG(selection ? 'filter-transform-empty-selection' : 'filter-transform-empty'),
      };
    }
    const initTransform = {
